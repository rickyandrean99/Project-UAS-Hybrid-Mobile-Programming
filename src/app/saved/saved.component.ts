...
export class SavedComponent implements OnInit {
  ...
  savedList() {
    this.ps.getSavedList(this.login).subscribe(
      (data) => {
        this.list = data
      ...
      buttons: [{
        ...
        handler: async () => {
          const confirm = await this.alert.create({
            header: 'Remove this post?',
            message: 'This post will be removed from saved list',
            buttons: [
              {
                text: 'Cancel',
                role: 'cancel',
              ...
              },
              {
                text: 'Okay',
                handler: async () => {
                  this.ps.changeSavedStatus(this.list[index].id, this.login).subscribe(
                    (data) => {
                      if (data == 'remove') {
